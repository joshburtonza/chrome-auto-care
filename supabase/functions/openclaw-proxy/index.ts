...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Raw SQL action (admin only, OPENCLAW_API_KEY required) ───────────────
    if (action === "sql") {
      const { query, params: sqlParams } = body;
      if (!query || typeof query !== "string") {
        return new Response(JSON.stringify({ error: "query is required for sql action" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      try {
        ...
        const rows = sqlParams?.length ? await sql.unsafe(query, sqlParams) : await sql.unsafe(query);
        await sql.end();
    ...
    if (!action || !table) {
      return new Response(JSON.stringify({ error: "action and table are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    switch (action) {
      ...
      case "update": {
        if (!data || !filters) {
          return new Response(JSON.stringify({ error: "data and filters are required for update" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "delete": {
        if (!filters) {
          return new Response(JSON.stringify({ error: "filters are required for delete (no mass deletes)" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "rpc": {
        ...
        if (!function_name) {
          return new Response(JSON.stringify({ error: "data.function_name is required for rpc" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (!allowedRpcs.includes(function_name)) {
          return new Response(JSON.stringify({ error: `RPC '${function_name}' is not allowed` }), {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      default:
        return new Response(
          JSON.stringify({
            error: `Unknown action: ${action}. Use select, insert, update, upsert, 
  delete, or rpc`,
          }),
