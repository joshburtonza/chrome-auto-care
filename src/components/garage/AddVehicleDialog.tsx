...
import { Plus, X, Image as ImageIcon, ChevronsUpDown, Check, Camera, Loader2 } from 'lucide-react';
  ...
  const handleScanLicenceDisc = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error('Failed to scan licence disc. Please fill in details manually.');
        return;
      }

      if (!data?.success || !data?.data) {
        toast.error(data?.error || 'Could not read licence disc. Please fill in details manually.');
        return;
      }
      ...
      if (result.year) {
        newFormData.year = result.year;
        fieldsFound++;
      }
      if (result.make) {
        newFormData.make = result.make;
        fieldsFound++;
      }
      if (result.model) {
        newFormData.model = result.model;
        fieldsFound++;
      }
      if (result.color) {
        newFormData.color = result.color;
        fieldsFound++;
      }
      if (result.vin) {
        newFormData.vin = result.vin;
        fieldsFound++;
      }
      ...
      if (fieldsFound > 0) {
        ...
        toast.warning('Could not extract any details. Please fill in manually.');
      }
